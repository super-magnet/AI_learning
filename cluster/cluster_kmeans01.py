...
# 为画图而定义的函数,使区间范围前后各增加0.1倍，用来定义坐标轴的取值区间
def expand(a,b):
    d = (b-a)*0.1
    return a-d, b+d
...
if __name__ == '__main__':
    ...
    #ds.make_blobs用于创建聚类模拟时的数据
    data2, y2 = ds.make_blobs(N, n_features=2, centers=centers, cluster_std=(1, 2.5, 0.5, 2), random_state=2)
    data3 = np.vstack((data[y==0][:], data[y==1][:50], data[y==2][:20], data[y==3][:5]))
    y3 = np.array([0]*100 + [1]*50 + [2]*20 + [3]*5)
    # print(data)
    #print(data[:,0]) #多维数据切片操作，例如此处代表切取第一列、所有行的数据
    # print(data[:][0]) #此处代表切取切取二维数组第1个向量所有数据
    # print(data[0][:]) #此处也代表切取切取二维数组第1个向量所有数据
    # print(data[0][1])
    # print(data[1][0])
    # print(data[y==0][:])
    ...
    cm = matplotlib.colors.ListedColormap(list('rgbm')) #定义一个颜色库，供后面画图使用
