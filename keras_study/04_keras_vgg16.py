# 使用迁移学习的思想，以VGG16作为末班搭建模型，训练识别手写字体
# 引入VGG16模块
...
# 加载OpenCV（在命令行中窗口中输入pip install opencv-python)，这里为了后期对图像进行处理
...
# 建立一个模型，其类型是Keras的Model类型，我们构建的模型会将VGG顶层去掉，只保留其余的网络
# 结构。这里用include_top = False表明我们迁移除顶层以外的其余网络结构到自己的模型中
# VGG模型对于输入图像数据要求至少为48个像素点，由于硬件配置限制，我们选用48个像素点，而不是
# VGG16所采用的224个像素点。即使这样仍然需要24GB以上的内存，或使用数据生成器
...
# 新的模型不需要训练原有卷积结构里面的1471万个参数，但是注意参数还是来自最后输出层的前两个
# 全连接层，一共有1.2亿个参数需要训练
sgd = SGD(lr=0.05, decay=1e-5)
model_vgg_mnist.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])

# 因为VGG16对网络输入层的要求，我们用OpenCV把图像从32*32变成224*224， 把黑白图像转换为RGB图像
...
model_vgg_mnist.fit(X_train, y_train_ohe, validation_data=(X_test, y_test_ohe), epochs=1, batch_size=50)
