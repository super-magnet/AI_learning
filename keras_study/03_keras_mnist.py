...
# 搭建卷积神经网路
model = Sequential()
# 添加一层卷积分，构造64个过滤器，每个过滤器覆盖范围是3*3*1
# 过滤器步长为1，图像四周补一圈0，用relu进行非线性变化
...
# 定义损失函数，一般来说分类问题的损失函数都采用交叉熵
model.compile(loss='categorical_crossentropy', optimizer='adagrad', metrics=['accuracy'])

# 放入批量样本，进行训练
model.fit(X_train, y_train_ohe, validation_data=(X_test, y_test_ohe), epochs=1, batch_size=128)
